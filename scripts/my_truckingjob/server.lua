...
local function LoadPlayerData(_source)
    local data = exports['my_datamanager']:GetPlayerDataKey(_source, 'trucking')
    if not data or not data.level then
        data = { level = 1, jobsDone = 0, totalEarnings = 0, dailyJobsCount = 0, lastJobDay = GetCurrentDay() }
        exports['my_datamanager']:SetPlayerDataKey(_source, 'trucking', data)
    end
    return data
end
...
RegisterNetEvent('my_truckingjob:requestPlayerData')
AddEventHandler('my_truckingjob:requestPlayerData', function()
    local _source = source
    local data = LoadPlayerData(_source)
    ResetDailyCountIfNewDay(data)
    SavePlayerData(_source, data)
    TriggerClientEvent('my_truckingjob:playerDataResult', _source, data)
end)

RegisterNetEvent('my_truckingjob:requestStartJob')
AddEventHandler('my_truckingjob:requestStartJob', function()
    local _source = source
    local data = LoadPlayerData(_source)
    ResetDailyCountIfNewDay(data)
    SavePlayerData(_source, data)

    local allowed = data.dailyJobsCount < Config.MaxDailyJobs
    TriggerClientEvent('my_truckingjob:startJobResult', _source, allowed, data)
end)

RegisterNetEvent('my_truckingjob:jobCompleted')
AddEventHandler('my_truckingjob:jobCompleted', function()
    local _source = source
    local data = LoadPlayerData(_source)
    ResetDailyCountIfNewDay(data)
    ...
    for lvl, v in pairs(Config.Levels) do
        if lvl <= 5 and data.jobsDone >= v.jobsRequired then
    ...
    TriggerClientEvent('my_truckingjob:jobCompleteClient', _source, payment, leveledUp, data)
    TriggerClientEvent('chatMessage', _source, "^2[Trucking]^7", "", "You received $" .. payment .. " for this delivery!")
