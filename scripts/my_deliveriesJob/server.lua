...
local function LoadPlayerData(_source)
    local data = exports['my_datamanager']:GetPlayerDataKey(_source, 'deliveries')
    if not data or not data.level then
        data = { level = 1, jobsDone = 0, deliveriesDone = 0, totalEarnings = 0, dailyJobsCount = 0, lastJobDay = GetCurrentDay() }
        exports['my_datamanager']:SetPlayerDataKey(_source, 'deliveries', data)
    end
    return data
end
...
-- Player requests their data (for menu)
RegisterNetEvent('my_deliveriesJob:requestPlayerData')
AddEventHandler('my_deliveriesJob:requestPlayerData', function()
    local _source = source
    local data = LoadPlayerData(_source)
    ResetDailyCountIfNewDay(data)
    SavePlayerData(_source, data)
    TriggerClientEvent('my_deliveriesJob:playerDataResult', _source, data)
end)

-- Start job
RegisterNetEvent('my_deliveriesJob:startJob')
AddEventHandler('my_deliveriesJob:startJob', function()
    local _source = source
    local data = LoadPlayerData(_source)
    ResetDailyCountIfNewDay(data)
    SavePlayerData(_source, data)
    ...
    if data.dailyJobsCount >= Config.MaxDailyJobs then
        TriggerClientEvent('my_deliveriesJob:startJobDenied', _source, data)
        return
    end

    -- Pick random delivery points
    local deliveryPoints = {}
    local allPoints = {}
    for i = 1, #Config.DeliveryPoints do
        table.insert(allPoints, Config.DeliveryPoints[i])
    end

    local numPoints = Config.RequiredDeliveries
    if #allPoints < numPoints then numPoints = #allPoints end

    for i = 1, numPoints do
        local randIndex = math.random(1, #allPoints)
        table.insert(deliveryPoints, allPoints[randIndex])
        table.remove(allPoints, randIndex)
    end

    TriggerClientEvent('my_deliveriesJob:startJobClient', _source, deliveryPoints, data)
end)

-- Individual delivery complete
RegisterNetEvent('my_deliveriesJob:deliveryComplete')
AddEventHandler('my_deliveriesJob:deliveryComplete', function()
    local _source = source
    local data = LoadPlayerData(_source)
    data.deliveriesDone = data.deliveriesDone + 1
    SavePlayerData(_source, data)
end)

-- Entire job complete
RegisterNetEvent('my_deliveriesJob:jobComplete')
AddEventHandler('my_deliveriesJob:jobComplete', function(numDeliveries)
    local _source = source
    local data = LoadPlayerData(_source)
    ResetDailyCountIfNewDay(data)
    ...
    TriggerClientEvent('my_deliveriesJob:jobCompleteClient', _source, payment, leveledUp, data)
    TriggerClientEvent('chatMessage', _source, "^2[Delivery]^7", "", "Job complete! Earned $" .. payment)
...
-- Cancel job
RegisterNetEvent('my_deliveriesJob:cancelJob')
AddEventHandler('my_deliveriesJob:cancelJob', function()
