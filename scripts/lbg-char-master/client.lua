...
-- Character data is now loaded from the server via my_datamanager
-- See 'lbgchar:loadCharacter' and 'playerSpawned' event handlers below

if Character["gender"] == "Female" then
	mdhash = GetHashKey("mp_f_freemode_01")
end

-- Male outfits: each index matches the outfit name list
-- Components: [3]=Torso, [4]=Pants, [6]=Shoes, [8]=Undershirt, [11]=Top
local moutfits = {
	{ -- 1: Business Suit (suit jacket, white shirt, dress pants, dress shoes)
		[3] = {4, 0},
		[11] = {4, 0},
		[8] = {10, 0},
		[4] = {10, 0},
	...
	{ -- 3: Sporty (track jacket, joggers, trainers)
		[3] = {5, 0},
		[11] = {5, 0},
		[8] = {15, 0},
		[4] = {5, 0},
...
-- Female outfits: matching index pairs to avoid clipping
local foutfits = {
	{ -- 1: Business Suit (suit jacket, white blouse, dress pants, heels)
		[3] = {7, 0},
		[11] = {7, 0},
		[8] = {2, 0},
		[4] = {3, 0},
	...
	{ -- 3: Sporty (zip jacket, leggings, trainers)
		[3] = {5, 0},
		[11] = {5, 0},
		[8] = {15, 0},
		[4] = {5, 0},
...
function EndCharCreator()
    ...
    FreezeEntityPosition(PlayerPedId(), false)
	Wait(1000)
	TriggerServerEvent('lbgchar:saveCharacter', Character)
	
	-- Teleport to default spawn location from my_position config
	local playerPed = PlayerPedId()
	SetEntityCoords(playerPed, -515.5311, -254.5744, 35.6268, false, false, false, true)
...
-- ============================================================
-- Character Data Loading / Saving (my_datamanager integration)
-- ============================================================
local isCharacterLoaded = false

-- Receive saved character data from server
...
AddEventHandler('lbgchar:loadCharacter', function(data)
    if data and type(data) == "table" then
        ...
        Character["lcgd"] = string.lower(Character["gender"])
        RefreshModel()
        ChangeComponents(false)
        isCharacterLoaded = true
        ShowNotification("~g~Character loaded.")
    end
end)

-- No saved character found — open the creator
RegisterNetEvent('lbgchar:noCharacter')
AddEventHandler('lbgchar:noCharacter', function()
    isCharacterLoaded = false
    ShowNotification("~y~Welcome! Create your character.")
    Citizen.Wait(1000)
    TriggerEvent('lbg-openChar')
end)

-- Server acknowledged save
RegisterNetEvent('lbgchar:saveComplete')
AddEventHandler('lbgchar:saveComplete', function(success)
    if success then
        isCharacterLoaded = true
        ShowNotification("~g~Character saved.")
    else
        ShowNotification("~r~Failed to save character.")
    end
end)

-- Request character on first spawn
AddEventHandler('playerSpawned', function()
    if not isCharacterLoaded then
        TriggerServerEvent('lbgchar:requestCharacter')
    else
        -- Re-apply on respawn (after death, etc.)
        RefreshModel()
        ChangeComponents(false)
    end
end)
